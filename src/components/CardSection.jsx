  ...
  return (
    <section className="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
      ...
      {  latestProducts.map((product,index) => (
        ...
        <section className={`p-5 py-10 text-center transform duration-500 hover:-translate-y-2 cursor-pointer ${index===0 ? "bg-pink-50" : index===1 ? "bg-green-50": index===2 ? "bg-blue-50" : "bg-red-50"}`}>
          <div className=" m-auto flex items-center justify-center ">
            <img src={product.image} alt="" className="object-cover w-[311px] h-[320px] max-sm:w-[195px] max-sm:h-[188px] rounded-md" />
          </div>
          
          <h1 className="text-3xl my-5 capitalize line-clamp-1">{product.name}</h1>
          ...
          <span className="font-semibold text-red-600 ">{`(${product.discount}% OFF)`}</span>
          <div className="flex space-x-2 justify-center items-center mb-3">
          <span className="font-semibold text-gray-800 text-xl">₹{product.discountedPrice }</span> 
